...
def adversarial_attack(x, m):
    input_shape = (28, 28, 1)
    input_tensor = Input(shape=input_shape)

    lenet_model = lenet5(input_tensor=input_tensor)
    fmodel = KerasModel(lenet_model, bounds=(0, 1))
    label = np.argmax(fmodel.predictions(x))

    attack = eval(mr)(fmodel)
    x_attacked = attack(x, label)
    ...
    def random_transform(self, x, seed=None):
        ...
        if self.light:
            if np.random.random() < 0.5:
                ...
                x = ep.adjust_gamma(x, np.random.uniform(0.7, 1.05))
                x = np.expand_dims(x, axis=img_channel_axis)

        if self.dilation:
            if np.random.random() < 0.5:
                x = x.reshape(28, 28)
                x = mp.dilation(x, mp.square(2, dtype=np.uint8))
                x = np.expand_dims(x, axis=img_channel_axis)
        ...
        if self.deepfool:
            if np.random.random() < 0.5:
                x_copy = x.copy()
                x = adversarial_attack(x, 'deepfool')
                if x is None:
                    x = x_copy 
        ...
        if self.spnoise:
            if np.random.random() < 0.5:
                x_copy = x.copy()
                x = adversarial_attack(x, 'spnoise')
                if x is None:
                    x = x_copy 
